...
HOOK = """
  window.__pg = {
    ...
    musicNow: function(){ return { playing: musicPlayer.playing, source: musicPlayer.source || null, box: musicPlayer.tune ? musicPlayer.tune.bpm : null, level: worldSettings.music ? worldSettings.music.bpm : null }; },
    boxTune: function(id, tune){ var g = gadgetById(id); if (!g) return false; g.tune = tune ? normalizeMusic(tune) : null; return !!g.tune; },
    matGroup: function(id){ if (id) { pmMatGroup = id; localStorage.setItem("pg_mat_group", id); } return pmMatGroup; },
    emote: function(){ return activeEmote && performance.now() < emoteUntil ? activeEmote : null; },
    dash: function(){ return me ? { until: me.dashUntil || 0, ang: me.dashAng, now: performance.now() } : null; },
    myTunes: function(){ return myTunes.map(function(t){ return { id: t.id, name: t.name, bpm: t.data.bpm }; }); },
    keepTune: function(name){ var m = worldSettings.music; if (!m) return 0; myTunes.unshift({ id: "tn" + Date.now(), name: name, data: JSON.parse(JSON.stringify(normalizeMusic(m))) }); persistMyTunes(); return myTunes.length; },
    useTune: function(i){ var t = myTunes[i]; if (!t) return false; musicStop(); worldSettings.music = normalizeMusic(t.data); return true; },
    newTune: function(){ worldSettings.music = newMusic(); return true; },
    doorSkin: function(id, skin){ var g = gadgets.filter(function(q){ return q.id === id; })[0]; if (!g) return null; if (skin !== undefined) g.skin = skin ? normalizeSkin(skin, "door") : null; return g.skin ? skinHasArt(g.skin) : false; },
    ...
    gadgets: function(){ return gadgets.map(function(g){ var wp = gadgetWorld(g); return { id:g.id, kind:g.kind, x:Math.round(wp.x), y:Math.round(wp.y), layer:g.layer,
      ...
      firing: g.firing, strength: g.strength, angle: g.angle, leadsTo: g.leadsTo || null, targetName: g.targetName, fireKind: g.fireKind, pgrav: g.pgrav, plife: g.plife, pbreaks: g.pbreaks, phurts: g.phurts, points: g.points, given: g.given, min: g.min, max: g.max, hush: g.hush, hasTune: !!(g.tune && musicHas(g.tune)), hits: g.hits, hitCount: g.count, fires: g.fires, only: g.only, useZone: g.useZone, useSpots: g.useSpots, destroy: g.destroy, spotArea: g.kind === 'projsensor' && g.weak ? +Math.abs(pgArea(g.weak)).toFixed(1) : 0,
