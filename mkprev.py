...
HOOK = """
  window.__pg = {
    ...
    saveSelectedAs: function(name){ var o = selList()[0]; if (!o) return false; var snap = o.pieces.map(function(pc){ return { m: pc.m, g: pgPack(pc.poly) }; }); savedObjects.push({ id: "u" + Date.now(), name: name, pieces: snap }); persistSavedObjects(); return true; },
    emitterUse: function(id, savedId){ var g = gadgetById(id), sv = savedObjects.find(function(s){ return s.id === savedId; }); if (!g || !sv) return false; g.emit = { name: sv.name, pieces: savedObjectPieces(sv).map(function(pc){ return { m: pc.m, g: pgPack(pc.poly) }; }) }; return true; },
