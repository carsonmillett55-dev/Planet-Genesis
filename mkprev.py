...
HOOK = """
  window.__pg = {
    ...
    gadgets: function(){ return gadgets.map(function(g){ var wp = gadgetWorld(g); return { id:g.id, kind:g.kind, x:Math.round(wp.x), y:Math.round(wp.y), layer:g.layer,
      ...
      firing: g.firing, strength: g.strength, angle: g.angle, fireKind: g.fireKind, pgrav: g.pgrav, plife: g.plife, pbreaks: g.pbreaks, phurts: g.phurts, hits: g.hits, hitCount: g.count, fires: g.fires, only: g.only, useZone: g.useZone, useSpots: g.useSpots, destroy: g.destroy, spotArea: g.kind === 'projsensor' && g.weak ? +Math.abs(pgArea(g.weak)).toFixed(1) : 0,
